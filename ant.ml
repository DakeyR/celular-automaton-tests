...
let show game = 
    Array.iteri (fun x col -> Array.iteri (fun y cell ->
        (if cell = 1 then set_color black
            else set_color white;
        fill_rect (x * 5)  (y * 5) 5 5); set_color black; draw_rect (x * 5) (y *5) 5 5) col) game;;
...
let do_step game ant ant_dir = 
    ...
    begin
        ...
        else turn_left ant_dir;
        game.(ant.(0)).(ant.(1)) <- (curr + 1) mod 2;
